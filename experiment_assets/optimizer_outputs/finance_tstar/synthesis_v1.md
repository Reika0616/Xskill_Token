...
**H1 — Consolidate the full calculation chain into one `code_interpreter` pass (including rounding/formatting).**
- Source: A (Contrast 2, Hyp 2), B (Contrast 1, Hyp 2), C (Contrast B, Hyp 2 + Hyp 3)
- S0 anchor: "Once all values obtained → code_interpreter for calculations"; "Use for all multi-step arithmetic"; "Round to reasonable precision" placed only in §6.
- Support: all three diagnoses, independently, with concrete within-executor contrasts (1 pass vs 2 passes).
- Conflicts: none.
- Risk: very low — organizational, not algorithmic; "show work" is preserved via comments/prints.
- Decision: **ACCEPT (merge C-H3 into it)**

**H2 — Constrain re-computation/validation-by-recalculation to detected discrepancies.**
- Source: A (Issue 2, Hyp 2), B (Hyp 3)
- S0 anchor: "Recalculate using a different method if possible"; "Double-check calculations".
- Support: two diagnoses; A observed a second pass motivated by rounding preference, B observed a pre-computation validation pass.
- Conflicts: partial tension with A-Hyp2's own request for magnitude cross-checks — resolved by keeping the cross-check but making it non-tool-invoking / in-block.
- Risk: medium if written as "trust your first result"; low if written as "cross-check, then recompute only on a detected discrepancy". Written the conservative way.
- Decision: **ACCEPT (tightened, correctness-preserving)**

**H3 — Labeled, printed values (totals, subtotals, final amounts) are extracted, not derived.**
- Source: B (Hyp 1)
- S0 anchor: "Use for all multi-step arithmetic (… totals)"; "once all values obtained" (ambiguous between read and computed).
- Support: single diagnosis, but strong contrast, explicit anchor, low risk.
- Conflicts: none. Consistency check with component sums retained inside the single pass, so no verification safeguard is lost.
- Risk: low (misread risk mitigated by keeping the in-block consistency check).
- Decision: **ACCEPT (merged with H1)**

**H4 — Attempt direct reading before zooming; zoom on demonstrated illegibility.**
- Source: C (Contrast A, Hyp 1), A (Hyp 1, same direction)
- S0 anchor: "Use zoom strategically: when exact numerical values are small or unclear" — subjective trigger, no read-first step.
- Support: two diagnoses. B neutral (zoom constant), not contradictory.
- Risk: low; zoom remains available as fallback and remains required when precision is genuinely at stake.
- Decision: **ACCEPT** (generalized to all visual evidence, not receipts only, to preserve S0 scope)

**H5 — Zoom stopping rule: once a value is read clearly, record and reuse it.**
- Source: A (Issue 1, Hyp 1)
- S0 anchor: "Re-zoom if the first magnification is insufficient"; "Avoid over-zooming beyond useful detail" — no sufficiency criterion.
- Support: single diagnosis, strong contrast (4 vs 6 vs 9 zooms, identical values); C's zoom evidence points the same direction.
- Risk: low — extraction success is observable.
- Decision: **ACCEPT**

**H6 — Name the value and its semantic location before zooming; confirm outcome after.**
- Source: A (Issue 3, Hyp 3)
- S0 anchor: "Identify the region containing the target information"; "Target the specific region".
- Support: single diagnosis, medium confidence, no conflicting evidence, purely additive to planning.
- Risk: very low — changes navigation planning, not what is extracted.
- Decision: **ACCEPT (merged into the zoom policy)**

**H7 — Always use `Decimal` for currency.**
- Source: A (rejected internally)
- Decision: **REJECT** — no correctness benefit shown; replaced by "fix the rounding convention up front".

## B. Accepted Changes

1. **Single calculation pass** (H1, H3, C-H3)
 - Evidence: A/B/C converging contrasts.
 - Anchor: §3 `code_interpreter`, Tool sequencing step 3, §6 rounding.
 - Policy: gather all inputs, then compute intermediates, final value, and final rounding in one block with comments and prints; consistency checks (component sum vs labeled total) live in that same block; labeled printed amounts are extracted rather than recomputed; a further pass only when new information arrives or a discrepancy is found.
 - Safeguard: "show work explicitly" retained and strengthened (prints for every intermediate); the sum-vs-labeled-total check retains B1's validation value.

2. **Validation as cross-check, recomputation on cause** (H2)
 - Evidence: A, B.
 - Anchor: §5 "Recalculate using a different method if possible", Output Principles "Double-check calculations".
 - Policy: cross-check magnitude, units, sign, and percentage base against the visual evidence; on a mismatch, name the cause and re-read or recompute only the affected part.
 - Safeguard: all S0 error checks (base, sign, units, scale) preserved; only aimless re-runs of a consistent result are removed.

3. **Read-first, then targeted zoom, with a stopping rule** (H4, H5, H6)
 - Evidence: A, C.
 - Anchor: §2 "Use zoom strategically", §3/Tool Guidance zoom blocks.
 - Policy: name the value needed and its semantic location; read at given resolution first; zoom when the value is not confidently readable; after each zoom record the value or the specific obstacle; once read clearly, retain and reuse; re-inspect only on ambiguity, conflict, or an implausible downstream result.
 - Safeguard: zoom triggers for genuine precision cases (decimals, currency symbols, overlapping series, dense legends, OHLC bodies) kept intact.

4. **Merge duplicated tool guidance** (consistency after edits 1–3)
 - S0 stated zoom / `code_interpreter` / `web_search` / `image_search` rules twice (§3 and "Tool Guidance"). Merged into one section so the new policies exist in exactly one place and cannot drift.

## C. Rejected Changes

- Fixed cap on zoom calls (A) — image difficulty varies; correctness risk.
- Mandating `Decimal` for currency (A) — no demonstrated benefit.
- "Read entire document in one pass, never zoom" / "always zoom" (A, C) — unsupported extremes.
- Reducing reasoning depth, explanation length, or intermediate-step reporting (B, C) — no evidence; harms transparency.
- Prescribing extraction order (B) — no evidence.
- Avoiding `code_interpreter` for financial arithmetic (C) — contradicts correctness safeguards.
- Mandating validation-by-recomputation (A, B) — contradicted by evidence.
- Forbidding directional language in zoom reasoning (A) — over-restrictive.
- "Stop after the first extracted value" (A) — misreads the multi-value requirement.

## D. Rewrite Plan

- **Description / When to Use / Strategy Overview**: preserved; Strategy Overview tightened so extraction, single-pass computation, and cross-check ordering are visible up front.
- **§1 Understand the Task**: preserved, compressed (domain lists kept).
- **§2 Inspect and Localize**: preserved; "Use zoom strategically" replaced by the read-first / named-target / stopping-rule extraction discipline.
- **§3 Select and Use Tools**: merged with the later "Tool Guidance" section; single-pass calculation policy and extraction-vs-derivation rule encoded here; tool sequencing tightened; example de-specified.
- **§4 Integrate Evidence**: preserved (units, precision, multi-source, temporal, tax/discount safeguards all kept).
- **§5 Validate**: replaced "recalculate if possible" with cross-check-then-diagnose; all error checks retained.
- **§6 Final Answer** + **Output Principles**: merged; rounding convention moved so it is fixed before computation.
- **Common Failure Modes**: preserved, compressed.
